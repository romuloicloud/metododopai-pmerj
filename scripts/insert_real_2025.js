...
async function insertExam() {
    try {
        const jsonPath = path.resolve(__dirname, '../extracted_questions_2025_real.json');
        ...
        console.log("🧹 Removendo dados antigos da prova 2025...");
        await supabase.from('questoes').delete().eq('exam_id', 'cp2-2025');

        console.log(`🚀 Inserindo Prova Real 2025 (${questions.length} questões)...`);
        ...
        if (error) {
            ...
            console.log(`✅ Sucesso! ${data.length} questões inseridas com ID 'cp2-2025'.`);
        }
