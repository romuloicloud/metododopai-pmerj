...
async function insertExam() {
    try {
        const jsonPath = path.resolve(__dirname, '../extracted_questions.json');
        ...
        console.log(`🚀 Iniciando inserção da prova CP2 2025 (${questions.length} questões)...`);
        ...
        const questionsToInsert = questions.map(q => ({
            ...
            text: q.enunciado,
            options: Array.isArray(q.alternativas) ? q.alternativas : Object.values(q.alternativas),
            correct_option_index: q.correct_option_index || 0, // Fallback if missing
